...
const {makeArray, isString, isFunction, isObject, isRegExp, lopper} = require("./util");
...
function _messageListeners(listeners, message, options) {
	listeners.forEach(listener=>listener(new Message(message, options)));
...
function _getPublishTo(listenerLookup, channels, message) {
	...
	for(const [listenerChannel, listeners] of [...matcher1, ...matcher2]) _filterListeners(listeners, message, publishTo);
	return publishTo;
}
...
function _getBroadcastTo(listenerLookup, channels, message) {
	...
	for (const [listenerChannel, listeners] of matcher) _filterListeners(listeners, message, broadcastTo);
...
function _mirrorMessageParse(message, parsers=[], base='') {
	makeArray(parsers).forEach(parser=>{
		message = parser(message, base);
	});

	message.target = makeArray(message.target || []).map(channel=>{
